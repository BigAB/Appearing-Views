...
@property (nonatomic, assign, getter = isDisappearing) BOOL disappearing;
...
+ (UIWindow *)mainWindow
{
    ...
    if (!window || ![window isMemberOfClass:[UIWindow class]]) window = [[UIApplication sharedApplication].windows objectAtIndex:0];
...
- (void)appear
{
    if (!self.hidden || self.isAppearing) return;
    self.appearing = YES;
...
- (void)disappear
{
    if (self.hidden || self.isDisappearing) return;
